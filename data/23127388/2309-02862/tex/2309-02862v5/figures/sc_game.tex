...
\begin{tikzpicture}[xscale=8,yscale=-3]
    ...
    \node (q1r1-A) at (0, 0) {$\tuple{ (\state_1, \rstate_1), \set{ x \mapsto 0 } }_A$};
    \node (q1r1-B) at (1, 0) {$\tuple{ (\state_1, \rstate_1), \set{ x \mapsto 0 } }_B$};

    \node (q2r1-B) at (0, 1) {$\tuple{ (\state_2, \rstate_1), \set{ x \mapsto 1 } }_B$};
    \node (q2r1-A) at (1, 1) {$\tuple{ (\state_2, \rstate_1), \set{ x \mapsto 1 } }_A$};

    \node[fin] (q2r2-A) at (0, 2) {$\tuple{ (\state_2, \rstate_2), \set{ x \mapsto 1 } }_A$};
    \node (q2r2-B) at (1, 2) {$\tuple{ (\state_2, \rstate_2), \set{ x \mapsto 1 } }_B$};
...
\end{tikzpicture}
\caption{
    The transition relation of the SC game $\game^\SC(\program, \set{\rstate_2})$ induced by the program $\program$ from \autoref{fig:concurrent-program}.
    Note that only configurations reachable from $\tuple{ (\state_1, \rstate_1), \set{ x \mapsto 0 } }_A$ are shown.
